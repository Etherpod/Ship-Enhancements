...
public class Tether : MonoBehaviour
{
    ...
    private float _reelSpeed = 5f;
    private readonly float _minTetherDistance = 0.25f;
    private float _maxTetherDistance = 1000f;
    ...
    public void CreateTether(OWRigidbody connectedBody, Vector3 anchorOffset, Vector3 connectedOffset, bool ignoreBuffer = false)
    {
        ...
        _tetherMesh = ShipEnhancements.CreateObject(ShipEnhancements.LoadPrefab("Assets/ShipEnhancements/Items/Tether/TetherLine.prefab"), transform).transform;
        ...
        if (attachedToPlayer)
        {
            ShipEnhancements.Instance.ModHelper.Events.Unity.FireOnNextUpdate(() => GlobalMessenger.FireEvent("SE_AttachPlayerTether"));
        }
    }
    ...
    public void DisconnectTether()
    {
        ...
        if (attachedToPlayer)
        {
            GlobalMessenger.FireEvent("SE_DetachPlayerTether");
        }
    }
